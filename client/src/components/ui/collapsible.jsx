...
export function Collapsible({ title, children, className }) {
  return (
    <CollapsiblePrimitive.Root className={cn('w-full overflow-hidden rounded-xl border border-border bg-card', className)}>
      <CollapsiblePrimitive.Trigger className="collapsible-trigger group flex min-h-12 w-full items-center justify-between px-4 py-3 text-left font-semibold outline-none hover:bg-secondary/60 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring">
        <span>{title}</span>
